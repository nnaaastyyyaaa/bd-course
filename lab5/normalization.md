...
В нашій схемі в сутності Shipment присутня delievery_address, що дублює в собі логіку сутності Address і є зайвим полем, бо Order і так знає про адресу, тому доцільніше зробити це через FK ?, а саме поле взагалі прибрати, бо воно лише ускладнює пошук та фільтрації по адресі, так як country, city та street знаходяться в одному рядку.

Інших атрибутів типу array та подібних, що можуть вміщати кілька значень одночасно, в нашій схемі немає. Кожен рядок є унікальний, таблиця не залежить від порядку рядків, типи даних в одній колонці не порушуються.
